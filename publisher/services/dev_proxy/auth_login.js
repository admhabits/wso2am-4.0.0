...
const callbackUrl = 'https://wso2am-admhabits.cloud.okteto.net:8081/publisher/services/auth/callback/login';
...
async function getSettings() {
    const res = await fetch('https://wso2am-admhabits.cloud.okteto.net:9443/api/am/publisher/v2/settings', { agent });
...
async function generateToken(code, keys) {
    ...
    const tokenEndpoint = 'https://wso2am-admhabits.cloud.okteto.net:9443/oauth2/token';
...
async function doDCR() {
    ...
    const dcrURL = 'https://wso2am-admhabits.cloud.okteto.net:9443/client-registration/v0.17/register';
...
function devServerBefore(app, server, compiler) {
    app.get('/publisher/services/auth/login', async (req, res, next) => {
        ...
        const location = 'https://wso2am-admhabits.cloud.okteto.net:9443/oauth2/authorize' + authRequestParams;
